...
void main() {
  ...
  setUp(() async {
    ...
    final product = Product(
      id: 1, sku: 'CAM-001', name: 'Camiseta Dry Fit',
      unit: 'UN', basePrice: 39.90, stock: 500,
    );
    final customer = Customer(id: 1, name: 'Loja Central', creditLimit: 15000, priceTableId: 1);
    ...
    await store.saveCatalog(products: [product], customers: [customer], rules: rules);

    app = AppState(ApiClient(baseUrl: 'http://127.0.0.1:1'), store); // porta morta = offline
  });
  ...
  test('pedido criado offline vai para a fila como pendente', () async {
    final c = app.customers.first;
    final p = app.products.first;
